...
class CreateMapVisual():
    def __init__(self, months, reduction, scope, use_months=True, pass_month_to_forward=False):
        self.months = months
    ...
    def __call__(self, pos_embedding, location_encoder, wb, section="test/"):
        ...
        if self.reduction == "first_three":
            ...
            if self.scope == "zur" and wb:
                try:
                    ...
                    log_dict[section + "zur_pca_first_3_dim_explained_variance"] = sum(pca.explained_variance_ratio_)
                    fig, ax = plt.subplots()
                    ax.plot(evrs)
        ...
        for i in range(len(self.months)):
            ...
            red_month = (red_month - red_month.min(axis=0)) / (red_month.max(axis=0) - red_month.min(axis=0))
            ...
            if wb:
                ...
                log_dict[section + str(self.months[i]) + "_" + self.reduction + "_" + self.scope] = img

        if wb:
            wb.log(log_dict)
        else:
            plt.imsave("./src/utils/test_cases/data/test_visual_" + self.scope + ".png", red_month)
