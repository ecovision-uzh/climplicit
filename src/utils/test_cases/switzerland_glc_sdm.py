...
class SW_PO_DS(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx): 
        ...
        return torch.tensor([datum["lon"], datum["lat"]]), int(datum["date"].split("-")[1]), datum["speciesId"]
...
class SDM(torch.nn.Module):
    def __init__(self, mlp_input_len, pos_embedding, location_encoder, use_months, pass_month_to_forward=False,
        hidden_dim=512, linear_probing=True):
        super().__init__()
        ...
        if linear_probing:
            ...
            layers = []
            layers += [nn.Linear(mlp_input_len, hidden_dim, bias=True), nn.ReLU()] # Input layer
    ...
    def forward(self, lonlat, month):
        ...
        if self.use_months:
            if self.pass_month_to_forward:
                ...
                loc = torch.concat([loc, torch.sin(month/12*torch.pi*2).unsqueeze(dim=-1),torch.cos(month/12*torch.pi*2).unsqueeze(dim=-1)], dim=-1)
...
class SW_SDM():
    def __init__(self, PO_path, mlp_input_len, use_months=False, verbose=False,
    most_common_species_map = False, pass_month_to_forward=False, linear_probing=False, iterations=1, train_loc_enc=False):
        self.use_months = use_months
    ...
    def run_(self, pos_embedding, location_encoder):
        ...
        self.train, self.val, self.test = torch.utils.data.random_split(po_ds, [0.7, 0.05, 0.25], generator=gn)
    ...
    def __call__(self, pos_embedding, location_encoder, wb, section="test/"):
        ...
        if wb:
            wb.log({"SW SDM/top1_mean" : top1s.mean(), "SW SDM/top5_mean" : top5s.mean(), "SW SDM/top10_mean" : top10s.mean(),
                    "SW SDM/top1_std" : top1s.std(), "SW SDM/top5_std" : top5s.std(), "SW SDM/top10_std" : top10s.std()})
        else:
            print(top1s.mean(), top5s.mean(), top10s.mean(), top1s.std(), top5s.std(), top10s.std())
            ...
            for m in ["03", "06", "09"]:
                ...
                for lonlat in dl:
                    ...
                    with torch.no_grad():
                        ...
                        encodings.append(model(lonlat, month).sigmoid().detach().cpu().float())
                ...
                im = ax.imshow(img, extent=(5.933, 10.514, 45.806, 47.813), vmin=0, vmax=1, cmap=plt.cm.plasma)
...
if __name__ == "__main__":
    ...
    sw_sdm = SW_SDM('/shares/wegner.ics.uzh/glc23_data/Switzerland_PO.csv', mlp_input_len=2, use_months=False, verbose=True, most_common_species_map=True, iterations=1)
