...
for month in months:
    encs = []
    raster = np.load("/shares/wegner.ics.uzh/CHELSA/Switzerland/1981-2010_numpy/" + month + "_monthly_float16.npy")
    for i in range(11):
...
x, y = (165,80)
len = 75
raster = np.load("/shares/wegner.ics.uzh/CHELSA/Switzerland/1981-2010_numpy/03_monthly_float16.npy")[:,y:y+len, x:x+len].transpose(1,2,0).reshape(len*len,11)
#encs = []
#for i in range(11):
#    encs.append(raster[i].reshape(245*550, 1))
