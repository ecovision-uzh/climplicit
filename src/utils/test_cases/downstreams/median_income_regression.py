...
class DS(torch.utils.data.Dataset):
    def __init__(self, unemployment_path, county_loc_path, norm_data=True):
        ...
        self.data = unemp.merge(
            county_locs, how="inner", left_on="FIPS_Code", right_on="fips_code"
        )
        self.data = self.data[["lng", "lat", "Median_Household_Income_2021"]].dropna()
        self.data = self.data.to_numpy().astype("float")
        if norm_data:
            self.data[:, 2] = (self.data[:, 2] - self.data[:, 2].mean()) / self.data[
                :, 2
            ].std()
    ...
    def __init__(
        ...
        if linear_probing:
            self.mlp = torch.nn.Linear(
                mlp_input_len if not use_months else mlp_input_len * 4, 1
            ).to("cuda")
        else:
...
class MIR:
    ...
    def __call__(self, pos_embedding, location_encoder, wb, section="test/"):
        ...
        if wb:
            wb.log(
                {
                    "SatCLIP Regressions/median_income_r2_mean": r2s.mean(),
                    "SatCLIP Regressions/median_income_r2_std": r2s.std(),
                }
            )
        else:
...
if __name__ == "__main__":
    ...
    reg = MIR(
        "/home/jdolli/chelsaCLIP/src/utils/test_cases/data/Unemployment.xlsx",
