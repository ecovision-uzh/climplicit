...
class DS(torch.utils.data.Dataset):
    def __init__(self, unemployment_path, county_loc_path, norm_data=True):
        unemp = pd.read_excel(unemployment_path, header=4)
        county_locs = pd.read_csv(county_loc_path)
        self.data = unemp.merge(county_locs, how = 'inner', left_on='FIPS_Code', right_on='fips_code')
        self.data = self.data[["lng", "lat", "Median_Household_Income_2021"]].dropna()
...
class MIR():
    def __init__(self, unemployment_path, county_loc_path, mlp_input_len, use_months, pass_month_to_forward=False, 
    verbose=True, linear_probing=True, iterations=1, train_loc_enc=False):
        ...
        self.county_loc_path = county_loc_path
    ...
    def run_(self, pos_embedding, location_encoder):
        ds = DS(self.unemployment_path, self.county_loc_path)
    ...
    def __call__(self, pos_embedding, location_encoder, wb, section="test/"):
        ...
        if wb:
            wb.log({"SatCLIP Regressions/median_income_r2_mean" : r2s.mean(), "SatCLIP Regressions/median_income_r2_std" : r2s.std()})
        else:
...
if __name__ == "__main__":
    ...
    reg = MIR('/home/jdolli/chelsaCLIP/src/utils/test_cases/data/Unemployment.xlsx',
    '/home/jdolli/chelsaCLIP/src/utils/test_cases/data/us_county_latlng.csv',mlp_input_len=4,
