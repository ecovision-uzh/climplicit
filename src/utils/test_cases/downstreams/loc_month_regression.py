...
class LMR():
    ...
    def run_(self, pos_embedding, location_encoder):
        ...
        if not self.train_loc_enc:
            train = torch.utils.data.DataLoader(dataset=self.train, batch_size=BATCH_SIZE, num_workers=4, shuffle=True)
            train_lonlat = []
            train_month = []
            train_x = []
            ...
            val = torch.utils.data.DataLoader(dataset=self.val, batch_size=BATCH_SIZE, num_workers=4, shuffle=True)
            val_lonlat = []
            val_month = []
            val_x = []
        ...
        for _ in EPOCHS:
            if not self.train_loc_enc:
                ...
                for idx in range(len(train_x)//BATCH_SIZE + 1):
                    ...
                    lonlat = train_lonlat[idx*BATCH_SIZE:(idx+1)*BATCH_SIZE].to("cuda")
                    month = train_month[idx*BATCH_SIZE:(idx+1)*BATCH_SIZE].to("cuda")
                    optimizer.zero_grad()
                    out = model.mlp(x).float().reshape(-1)
                    loss = loss_fn(out, torch.cat([lonlat, month.reshape(-1, 1)], dim=-1).reshape(-1))
                    loss.backward()
                    optimizer.step()
                ...
                for idx in range(len(val_x)//BATCH_SIZE + 1):
                    ...
                    lonlat = val_lonlat[idx*BATCH_SIZE:(idx+1)*BATCH_SIZE].to("cuda")
                    month = val_month[idx*BATCH_SIZE:(idx+1)*BATCH_SIZE].to("cuda")
                    with torch.no_grad():
                        out = model.mlp(x).float().reshape(-1)
                    acc_loss += loss_fn(out, torch.cat([lonlat, month.reshape(-1, 1)], dim=-1).reshape(-1)).detach().cpu()
                acc_loss /= len(val)
                model.mlp.train()
            else:
        ...
        model.eval()
        test = torch.utils.data.DataLoader(dataset=self.test, batch_size=BATCH_SIZE, num_workers=4, shuffle=False)
    ...
    def __call__(self, pos_embedding, location_encoder, wb, section="test/"):
        ...
        if wb:
            wb.log({"Loc-Month Regression/lonlat_r2_mean" : lonlat_r2s.mean(), "Loc-Month Regression/lonlat_r2_std" : lonlat_r2s.std(),
                "Loc-Month Regression/month_r2_mean" : month_r2s.mean(), "Loc-Month Regression/month_r2_std" : month_r2s.std()})
        else:
            print(lonlat_r2s.mean(), lonlat_r2s.std(), month_r2s.mean(), month_r2s.std())
