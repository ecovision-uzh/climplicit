...
class EU_SDM():
    ...
    most_common_species_map = False, pass_month_to_forward=False, linear_probing=True, iterations=1, train_loc_enc=False):
        self.use_months = use_months
    ...
    def __call__(self, pos_embedding, location_encoder, wb, section="test/"):
        ...
        if wb:
            wb.log({"EU SDM/top1_mean" : top1s.mean(), "EU SDM/top1_std" : top1s.std()})
        else:
            print(top1s.mean(), top1s.std())
        ...
        if self.most_common_species_map:
            ...
            for m in ["03", "06", "09"]:
                ...
                if wb:
                    ...
                    wb.log({"EU SDM/" + str(sid) + "_" + str(m): img})
                else:
                    fig.savefig("./temp_mcsm.png")
...
if __name__ == "__main__":
    ...
    #eu_sdm = EU_SDM('/shares/wegner.ics.uzh/glc23_data/Switzerland_PO.csv', mlp_input_len=2, use_months=False, verbose=True, most_common_species_map=True, iterations=1)
    eu_sdm = EU_SDM('/shares/wegner.ics.uzh/glc23_data/Pot_10_to_1000.csv', mlp_input_len=2, use_months=False, verbose=True, most_common_species_map=True, iterations=1)
    eu_sdm(pos_embedding, location_encoder, None)
