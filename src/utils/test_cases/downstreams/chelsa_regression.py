...
class DS(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        return torch.tensor(self.ptc[idx]), torch.tensor(
            self.chelsa[idx], dtype=torch.float
        )
    ...
    def __init__(
        ...
        if linear_probing:
            self.mlp = torch.nn.Linear(
                mlp_input_len if not use_months else mlp_input_len * 4, 11
            ).to("cuda")
        else:
...
class CHR:
    ...
    def run_(self, pos_embedding, location_encoder):
            ...
            if not self.train_loc_enc:
                ...
                for idx in range(len(train_x) // BATCH_SIZE + 1):
                    x, y = train_x[idx * BATCH_SIZE : (idx + 1) * BATCH_SIZE].to(
                        "cuda"
                    ), train_y[idx * BATCH_SIZE : (idx + 1) * BATCH_SIZE].to(
                        "cuda"
                    ).reshape(
                        -1
                    )
                ...
                for idx in range(len(val_x) // BATCH_SIZE + 1):
                    x, y = val_x[idx * BATCH_SIZE : (idx + 1) * BATCH_SIZE].to(
                        "cuda"
                    ), val_y[idx * BATCH_SIZE : (idx + 1) * BATCH_SIZE].to(
                        "cuda"
                    ).reshape(
                        -1
                    )
        ...
        max_chelsa = (
            ds.chelsa.max()
        )  # The maximum value of the image is unbound, thus just set to highest value in the original
        PSNR = (
            10
            * torch.log10(np.power(max_chelsa, 2) / (acc_loss / len(test)))
            .detach()
            .cpu()
        )
    ...
    def __call__(self, pos_embedding, location_encoder, wb, section="test/"):
        ...
        if wb:
            wb.log(
                {
                    "CHELSA Regression/chelsa_r2_mean": r2s.mean(),
                    "CHELSA Regression/chelsa_r2_std": r2s.std(),
                    "CHELSA Regression/chelsa_psnr_mean": psnrs.mean(),
                    "CHELSA Regression/chelsa_psnr_std": psnrs.std(),
                    "CHELSA Regression/chelsa_mse_mean": mses.mean(),
                    "CHELSA Regression/chelsa_mse_std": mses.std(),
                }
            )
        else:
            print(
