...
class GCR():
        ...
        for _ in EPOCHS:
            if not self.train_loc_enc:
                ...
                for idx in range(len(train_x)//BATCH_SIZE + 1):
                    ...
                    loss = loss_fn(out.reshape(-1), y.reshape(-1))
                    loss.backward()
                    optimizer.step()
    ...
    def __call__(self, pos_embedding, location_encoder, wb, section="test/"):
        ...
        if wb:
            wb.log({"Global Chelsa Regression/r2_mean" : r2s.mean(), "Global Chelsa Regression/r2_std" : r2s.std()})
        else:
...
if __name__ == "__main__":
    ...
    reg = GCR('/home/jdolli/chelsaCLIP/src/utils/test_cases/data/global_chelsa_100000.npy', mlp_input_len=4,
    use_months=True, verbose=True, iterations=1)
