...
class DS(torch.utils.data.Dataset):
    def __init__(self, file_path, norm_data=True):
        self.data = np.array(pd.read_csv(file_path))
        self.data[:,4] = (self.data[:,4] - self.data[:,4].mean()) / self.data[:,4].std()
        #self.data[:, 4] = self.data[:, 4]/self.data[:, 4].max()
    
    def __len__(self):
        return len(self.data)

    def __getitem__(self, idx): 
        return torch.tensor(self.data[idx, :2]), torch.tensor(self.data[idx, 4], dtype=torch.float)
...
class Probe(torch.nn.Module):
    def __init__(self, mlp_input_len, pos_embedding, location_encoder, use_months, pass_month_to_forward=False, 
        ...
        if linear_probing:
            ...
            #hidden_dim = hidden_dim, layers = 2, out_dim=1, batchnorm=False).to("cuda")
            layers = []
...
class ATR():
    ...
    verbose=False, linear_probing=True, iterations=1, train_loc_enc=False):
    ...
    def run_(self, pos_embedding, location_encoder):
        ...
        for _ in EPOCHS:
            ...
            if best_val_loss - acc_loss > 0.0001:
                ...
                es_counter += 1
            if self.verbose:
                EPOCHS.set_description("Val-loss: %.8f (Best: %.8f - Patience: %i)" % (loss, best_val_loss, es_counter))
    ...
    def __call__(self, pos_embedding, location_encoder, wb, section="test/"):
        ...
        if wb:
            wb.log({"SatCLIP Regressions/air_temp_r2_mean" : r2s.mean(), "SatCLIP Regressions/air_temp_r2_std" : r2s.std()})
        else:
...
if __name__ == "__main__":
    ...
    reg = Regression('/home/jdolli/chelsaCLIP/src/utils/test_cases/data/stationDataAll.csv', mlp_input_len=4,
