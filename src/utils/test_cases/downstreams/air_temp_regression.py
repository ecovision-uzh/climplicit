...
class DS(torch.utils.data.Dataset):
    def __init__(self, file_path, norm_data=True):
        ...
        self.data[:, 4] = (self.data[:, 4] - self.data[:, 4].mean()) / self.data[
            :, 4
        ].std()
    ...
    def __getitem__(self, idx):
        return torch.tensor(self.data[idx, :2]), torch.tensor(
            self.data[idx, 4], dtype=torch.float
        )
...
class ATR:
    ...
    def __call__(self, pos_embedding, location_encoder, wb, section="test/"):
        ...
        if wb:
            wb.log(
                {
                    "SatCLIP Regressions/air_temp_r2_mean": r2s.mean(),
                    "SatCLIP Regressions/air_temp_r2_std": r2s.std(),
                }
            )
        else:
