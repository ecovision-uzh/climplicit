    ...
    def __init__(
        ...
        if linear_probing:
            self.mlp = torch.nn.Linear(
                mlp_input_len if not use_months else mlp_input_len * 4, out_dim
            ).to("cuda")
        else:
...
class BEC:
    def __init__(
        ...
        self.vals_to_labels = {
            self.vals[i]: self.labels[i] for i in range(len(self.vals))
        }
    ...
    def __call__(self, pos_embedding, location_encoder, wb, section="test/"):
        ...
        if self.mode == "biomes":
            ...
            log_dict = {
                "BIOMES C/top1_mean": top1s.mean(),
                "BIOMES C/top1_std": top1s.std(),
            }
            for i in range(len(self.labels)):
                biome = self.labels[i]
                log_dict["BIOMES C/" + biome + "_f1_mean"] = (
                    biomes_means[i].item() * 100
                )
            ...
            wb.log(
                {
                    "ECOREGIONS C/top1_mean": top1s.mean(),
                    "ECOREGIONS C/top1_std": top1s.std(),
                }
            )
        ...
        if self.mode == "biomes" and self.plot_confusion_matrix:
            ...
            dl = torch.utils.data.DataLoader(
                dataset=ds, batch_size=8096, num_workers=0, shuffle=True
            )
            if self.verbose:
            ...
            def map_res(scope):
                    ...
                    with torch.no_grad():
                        if self.use_months:
                            ...
                            for m in [3, 6, 9, 12]:
                                ...
                                if self.pass_month_to_forward:
                                    temp.append(
                                        model.location_encoder(loc, month).float()
                                    )
                                else:
                                    loc_month = torch.concat(
                                        [
                                    ...
                                    temp.append(
                                        model.location_encoder(loc_month).float()
                                    )
                            ...
                            encodings.append(
                                model.mlp(model.location_encoder(loc).float())
                                .detach()
                                .cpu()
                            )
                ...
                if scope == "lv":
                    ...
                    for biome in [7, 11, 12, 13]:
                        ...
                        imgs[dl.dataset.land_mask != 0] = encodings[:, biome].reshape(
                            -1, 1
                        )
                        ax.set_xlabel("Longitude")
                        ax.set_ylabel("Latitude")
                        ...
                        if wb:
                            ...
                            img = wandb.Image(
                                PIL.Image.open("./temp_lvb_" + str(biome) + ".png")
                            )
                            wb.log(
                                {
                                    "BIOMES C/lake_victoria_"
                                    + self.vals_to_labels[biome]: img
                                }
                            )
                            img = wandb.Image(
                                PIL.Image.open("./temp_lvb_" + str(biome) + "_gt.png")
                            )
                            wb.log(
                                {
                                    "BIOMES C/lake_victoria_"
                                    + self.vals_to_labels[biome]
                                    + "_gt": img
                                }
                            )
                        else:
                ...
                if scope != "world":  # im = ax.scatter(x, y, c=img)
                    ...
                    for lonlat, y in self.train:
                        ...
                        if ds.W <= lon <= ds.E and ds.S <= lat <= ds.N:
                            ...
                            lats.append(lat)
                    plt.plot(
                        lons, lats, marker="+", ls="None", c="k", alpha=1, markersize=5
                    )
                    lons = []
                    lats = []
                ...
                for i in range(len(self.vals)):
                    if len(x[img == self.vals[i]]) > 0:
                        handles.append(
                            mpatches.Patch(color=colors[i], label=self.labels[i])
                        )
                handles.append(
                    mlines.Line2D(
                        [],
        ...
        if self.biome_tsne and self.mode == "biomes":
            ...
            lonlat = (
                torch.stack(
                    [
                        torch.tensor(sample["LON"].to_numpy()),
                        torch.tensor(sample["LAT"].to_numpy()),
                    ]
                )
                .transpose(1, 0)
                .to("cuda")
            ...
            ax.scatter(
                ...
                red[:, 1],
                marker="o",
                c=[val_to_color[int(i)] for i in sample["BIOME"]],
                alpha=1,
                s=40,
            )
...
class EcoregionsLocEnc(torch.nn.Module):
    def __init__(self):
        super().__init__()
        self.eco = gpd.read_file(
            "/home/jdolli/chelsaCLIP/src/utils/test_cases/data/data/commondata/data0/wwf_terr_ecos.shp"
        )
    ...
    def forward(self, x):
        ...
        pts_gdf = gpd.GeoDataFrame(
            pts, geometry=gpd.points_from_xy(pts["LON"], pts["LAT"], crs="EPSG:4326")
        )
...
if __name__ == "__main__":
    ...
    sw_sdm = BEC(
        "/home/jdolli/chelsaCLIP/src/utils/test_cases/data/ecobiomes_100000.csv",
        mode="biomes",
        mlp_input_len=100,
        use_months=False,
        verbose=True,
