...
class SigmoidClipLoss(torch.nn.Module):
    ...
    def forward(
        ...
        logits = (
            torch.matmul(chelsa_emb, loc_month_emb.t()) * self.t_prime.exp() + self.b
        )
        if verbose:
        ...
        if l_module:
            l_module.log(
                "train/diagonal loss", -torch.mean(torch.diagonal(loss_matrix))
            )
...
class SoftmaxClipLoss(torch.nn.Module):
    ...
    def forward(self, loc_month_emb, chelsa_emb, similarity=None, verbose=False):
        ...
        labels = torch.arange(
            len(logits_per_loc_month),
            device=logits_per_loc_month.device,
            dtype=torch.long,
        )
