...
class ChelsaCLIPDataset(Dataset):
    def __init__(
        ...
        if return_size > 1:
            self.y_len, self.x_len = self.monthly_arrays[
                list(self.monthly_arrays.keys())[0]
            ][0].shape
        ...
        if whiten_with_pca:
            self.whiten_pca = pk.load(
                open(
                    "/shares/wegner.ics.uzh/CHELSA/input/whiten_pca_components.pkl",
                    "rb",
                )
            )
        else:
    ...
    def get_idx(self, idx, month):
        ...
        if self.whiten_pca:
            chelsa = (
                self.whiten_pca.transform(chelsa.reshape((1, 11)))
                .reshape((11))
                .astype("float16")
            )
    ...
    def get_idx_future_clims(self, idx, month, tf, ssp):
        ...
        if self.whiten_pca:
            raise NotImplementedError(
                "Haven't calculated whitening for future climatologies yet"
            )
        ...
        for i in range(-1, 2):
            for j in range(-1, 2):
                ...
                if not (
                    ...
                    or x < 0
                    or x > shape[1] - 1
                    or (x == pixel_x and y == pixel_y)
                ):
                    lonlats.append(
                        torch.tensor(np.array([self.x_to_lon[x], self.y_to_lat[y]]))
                    )
    ...
    def __getitem__(self, idx):
        if self.local_multi_sampling:
            ...
            return self.get_idx_future_clims(
                idx,
                month=random.choice(self.months),
                tf=random.randint(0, 3),
                ssp=random.randint(0, 2),
            )
        else:
