...
refras =  rioxarray.open_rasterio(climatology_dir + "CHELSA_gfdl-esm4_r1i1p1f1_w5e5_ssp585_tasmin_07_2071_2100_norm.tif", cache=False)
ys = refras.y
xs = refras.x
try:
...
land_mask[ys<-63] = False  # CHELSA has no values below this point even though there is landmass

# These were calculated by taking mean/std across current-climate all months
var_mean = np.array([[[688.4960482]], [[2817.54503453]], [[2862.30705101]], [[2771.25053521]]])
var_std = np.array([[[875.89886103]], [[176.02514265]], [[183.50181411]], [[168.30246239]]])
all_months = []
for month in months:
    print("Month:", month)
    all_tfs = []
    for tf in tqdm(time_frames):
        ...
        for ssp in ssps:
            ...
            for var in var_names:
                if tf == "current":
                    raster_file_name = climatology_dir + "CHELSA_" + var + "_" + str(month) + "_1981-2010_V.2.1.tif"
                else:
                    # CHELSA_gfdl-esm4_r1i1p1f1_w5e5_ssp585_tasmin_07_2071_2100_norm.tif
                    raster_file_name = climatology_dir + "CHELSA_gfdl-esm4_r1i1p1f1_w5e5_" + \
                    ssp + "_" + var + "_" + month + "_" + tf + "_norm.tif"
