...
sharpness = np.memmap(
    "./sharpness.npy",
    dtype=np.half,
    shape=(raster[0].shape[-2], raster[0].shape[-1]),
    mode="w+",
)


def process(i, j, output, raster):
    raster_block = np.stack(
        [
            r[
                :,
    ...
    output[i * BLOCK_LEN : (i + 1) * BLOCK_LEN, j * BLOCK_LEN : (j + 1) * BLOCK_LEN] = (
        np.average(gnorm, axis=0)
    )


Parallel(n_jobs=os.cpu_count())(
    delayed(process)(i, j, sharpness, raster) for i, j in tqdm(block_ids)
)
...
sharpness[to_be_interpolated] = np.interp(
    x(to_be_interpolated), x(~to_be_interpolated), sharpness[~to_be_interpolated]
)
