...
normalization_file = "/shares/wegner.ics.uzh/CHELSA" + "/climatologies/1981-2010/" + "normalization_values.json"
...
try:
    for file_name in tqdm(norm.keys()):
        if not os.path.isfile(file_name.replace(old_climatology_name, new_climatology_name)):
            ...
            try:
                ...
                pass
            file = (file - mean) / std
            file.rio.to_raster(file_name.replace(old_climatology_name, new_climatology_name))
            file.close()
            del file
